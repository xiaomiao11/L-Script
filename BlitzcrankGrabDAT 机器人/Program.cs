...
namespace BlitzcrankGrabDAT
{
    class Program
    {
        ...
        private static void Game_OnGameLoad(EventArgs args)
        {
            ...
            //Create the menu
            menu = new Menu("Blitzcrank浜岀嫍姹夊寲", "Blitzcrank", true);
            ...
            menu.SubMenu("Keys").AddItem(new MenuItem("panic", "閫冭窇").SetValue(new KeyBind("Z".ToCharArray()[0], KeyBindType.Press)));
            //Combo menu:
            menu.AddSubMenu(new Menu("杩炴嫑", "Combo"));
            menu.SubMenu("Combo")
             .AddItem(
                 new MenuItem("tsModes", "妯″紡").SetValue(
                     new StringList(new[] { "璧扮爫", "浣巪HP%", "鎺ヨ繎榧犳爣", "褰撳墠HP" }, 0)));
            menu.SubMenu("Combo").AddItem(new MenuItem("selected", "鐩爣閫夋嫨").SetValue(true));
            menu.SubMenu("Combo").AddItem(new MenuItem("UseQCombo", "U浣跨敤 Q").SetValue(true));
            menu.SubMenu("Combo").AddItem(new MenuItem("qHit", "鑷姩Q浜烘暟").SetValue(new Slider(3, 1, 4)));
            ...
            menu.SubMenu("Combo").AddItem(new MenuItem("QE", "鑷姩E鎸戣捣").SetValue(true));
            menu.SubMenu("Combo").AddItem(new MenuItem("UseRCombo", "浣跨敤 R").SetValue(true));
            menu.SubMenu("Combo").AddItem(new MenuItem("useRQ", "Q鍚庤嚜鍔≧").SetValue(true));

            //Q Menu
            menu.AddSubMenu(new Menu("Q鑿滃崟", "qMenu"));
            menu.SubMenu("qMenu").AddItem(new MenuItem("qRange2", "Q鏈€灏忚寖鍥磡").SetValue(new Slider(300, 1, 950)));
            menu.SubMenu("qMenu").AddItem(new MenuItem("qRange", "Q鏈€澶ц寖鍥磡").SetValue(new Slider(900, 1, 950)));
            menu.SubMenu("qMenu").AddItem(new MenuItem("qSlow", "Q缂撴參鐩爣").SetValue(true));
            menu.SubMenu("qMenu").AddItem(new MenuItem("qImmobile", "Q涓嶅姩鐩爣").SetValue(true));
            menu.SubMenu("qMenu").AddItem(new MenuItem("qDashing", "Q绉诲姩鐩爣").SetValue(true));

            //Harass menu:
            menu.AddSubMenu(new Menu("楠氭壈", "Harass"));
            menu.SubMenu("Harass").AddItem(new MenuItem("UseQHarass", "浣跨敤 Q").SetValue(true));
            menu.SubMenu("Harass").AddItem(new MenuItem("qHit2", "鑷姩Q浜烘暟").SetValue(new Slider(3, 1, 4)));
            ...
            menu.SubMenu("Misc").AddItem(new MenuItem("resetE", "鑷姩AEA").SetValue(true));
            menu.SubMenu("Misc").AddItem(new MenuItem("autoR", "鑷姩AEA").SetValue(new Slider(3, 0, 5)));

            menu.SubMenu("Misc").AddSubMenu(new Menu("鏃燫涓峇", "intR"));
