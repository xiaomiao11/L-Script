...
namespace D_Kogmaw
{
    public static class Program
    {
        ...
        private static void Game_OnGameLoad(EventArgs args)
        {
            ...
            //D Kogmaw
            _config = new Menu("D-Kogmaw浜岀嫍姹夊寲", "D-Kogmaw", true);
            ...
            if (Skins.Count > 0)
            {
                _config.AddSubMenu(new Menu("鏇存崲鐨偆", "Skin Changer"));
                _config.SubMenu("Skin Changer")
                    .AddItem(new MenuItem("Skin_enabled", "浣跨敤鎹㈣偆").SetValue(false));
                _config.SubMenu("Skin Changer")
                    .AddItem(new MenuItem("Skin_select", "鐨偆閫夋嫨").SetValue(new StringList(Skins.ToArray())));
            ...
            _config.SubMenu("Combo").AddItem(new MenuItem("UseRC", "浣跨敤 R")).SetValue(true);
            _config.SubMenu("Combo").AddItem(new MenuItem("UseIgnitecombo", "浣跨敤鐐圭噧鍑绘潃")).SetValue(true);
            _config.SubMenu("Combo").AddItem(new MenuItem("RlimC", "浣跨敤R鏈€澶т汉鏁皘").SetValue(new Slider(3, 1, 5)));
            _config.SubMenu("Combo")
                .AddItem(new MenuItem("ActiveCombo", "杩炴嫑!").SetValue(new KeyBind(32, KeyBindType.Press)));

            _config.AddSubMenu(new Menu("鐗╁搧", "items"));
            _config.SubMenu("items").AddSubMenu(new Menu("鏀诲嚮鐗╁搧", "Offensive"));
            ...
            _config.SubMenu("items")
                .SubMenu("Offensive")
                .AddItem(new MenuItem("BilgeEnemyhp", "浣跨敤灏忓集鍒€鏁屼汉琛€閲弢<").SetValue(new Slider(85, 1, 100)));
            _config.SubMenu("items")
                .SubMenu("Offensive")
                .AddItem(new MenuItem("Bilgemyhp", "浣跨敤灏忓集鍒€鑷繁琛€閲弢< ").SetValue(new Slider(85, 1, 100)));
            _config.SubMenu("items").SubMenu("Offensive").AddItem(new MenuItem("Blade", "浣跨敤鐮磋触")).SetValue(true);
            _config.SubMenu("items")
                .SubMenu("Offensive")
                .AddItem(new MenuItem("BladeEnemyhp", "浣跨敤鐮磋触鏁屼汉琛€閲弢<").SetValue(new Slider(85, 1, 100)));
            _config.SubMenu("items")
                .SubMenu("Offensive")
                .AddItem(new MenuItem("Blademyhp", "浣跨敤鐮磋触鑷繁琛€閲弢<").SetValue(new Slider(85, 1, 100)));
            _config.SubMenu("items").SubMenu("Offensive").AddItem(new MenuItem("usedfg", "浣跨敤鍐ョ伀")).SetValue(true);
            _config.SubMenu("items").AddSubMenu(new Menu("闃插尽鐗╁搧", "Deffensive"));
            _config.SubMenu("items")
                .SubMenu("Deffensive")
                .AddItem(new MenuItem("Zhonyas", "浣跨敤涓簹"))
                .SetValue(true);
            _config.SubMenu("items")
                .SubMenu("Deffensive")
                .AddItem(new MenuItem("Zhonyashp", "浣跨敤涓簹琛€閲弢<").SetValue(new Slider(20, 1, 100)));
            ...
            _config.SubMenu("Harass").AddItem(new MenuItem("UseEH", "浣跨敤 E")).SetValue(true);
            _config.SubMenu("Harass").AddItem(new MenuItem("UseRH", "浣跨敤R")).SetValue(true);
            _config.SubMenu("Harass").AddItem(new MenuItem("RlimH", "浣跨敤R鏈€澶т汉鏁皘").SetValue(new Slider(2, 1, 5)));
            _config.SubMenu("Harass")
                .AddItem(
                    new MenuItem("harasstoggle", "楠氭壈 (鑷姩)").SetValue(new KeyBind("G".ToCharArray()[0],
                        KeyBindType.Toggle)));
            _config.SubMenu("Harass")
                .AddItem(new MenuItem("Harrasmana", "楠氭壈鏈€浣庤摑閲弢").SetValue(new Slider(65, 1, 100)));
            _config.SubMenu("Harass")
                .AddItem(
            ...
            _config.SubMenu("Farm")
                .SubMenu("Lasthit")
                .AddItem(new MenuItem("Lastmana", "琛ュ垁鏈€浣庤摑閲弢").SetValue(new Slider(65, 1, 100)));
            _config.SubMenu("Farm")
                .SubMenu("Lasthit")
                .AddItem(
                    new MenuItem("ActiveLast", "琛ュ垁！").SetValue(new KeyBind("X".ToCharArray()[0], KeyBindType.Press)));
            _config.SubMenu("Farm").AddSubMenu(new Menu("娓呭叺", "Laneclear"));
            _config.SubMenu("Farm").SubMenu("Laneclear").AddItem(new MenuItem("UseQL", "Q 娓呭叺")).SetValue(true);
            _config.SubMenu("Farm").SubMenu("Laneclear").AddItem(new MenuItem("UseEL", "E 娓呭叺")).SetValue(true);
            _config.SubMenu("Farm").SubMenu("Laneclear").AddItem(new MenuItem("UseRL", "R 娓呭叺")).SetValue(true);
            _config.SubMenu("Farm")
                .SubMenu("Laneclear")
                .AddItem(new MenuItem("RlimL", "浣跨敤R鏈€澶х洰鏍囨暟").SetValue(new Slider(1, 1, 5)));
            _config.SubMenu("Farm")
                .SubMenu("Laneclear")
                .AddItem(new MenuItem("Lanemana", "娓呭叺鏈€浣庤摑閲弢").SetValue(new Slider(65, 1, 100)));
            _config.SubMenu("Farm")
                .SubMenu("Laneclear")
                .AddItem(
                    new MenuItem("ActiveLane", "娓呴噹").SetValue(new KeyBind("V".ToCharArray()[0],
                        KeyBindType.Press)));
            _config.SubMenu("Farm").AddSubMenu(new Menu("娓呴噹", "Jungleclear"));
            _config.SubMenu("Farm").SubMenu("Jungleclear").AddItem(new MenuItem("UseQJ", "Q 娓呴噹")).SetValue(true);
            _config.SubMenu("Farm").SubMenu("Jungleclear").AddItem(new MenuItem("UseWJ", "W 娓呴噹")).SetValue(true);
            _config.SubMenu("Farm").SubMenu("Jungleclear").AddItem(new MenuItem("UseEJ", "E 娓呴噹")).SetValue(true);
            _config.SubMenu("Farm").SubMenu("Jungleclear").AddItem(new MenuItem("UseRJ", "R 娓呴噹")).SetValue(true);
            _config.SubMenu("Farm")
                .SubMenu("Jungleclear")
                .AddItem(new MenuItem("RlimJ", "浣跨敤R鏈€澶х洰鏍囨暟").SetValue(new Slider(2, 1, 5)));
            _config.SubMenu("Farm")
                .SubMenu("Jungleclear")
                .AddItem(new MenuItem("junglemana", "娓呴噹鏈€浣庤摑閲弢").SetValue(new Slider(60, 1, 100)));
            _config.SubMenu("Farm")
                .SubMenu("Jungleclear")
                .AddItem(
                    new MenuItem("Activejungle", "娓呭叺|娓呴噹").SetValue(new KeyBind("V".ToCharArray()[0],
                        KeyBindType.Press)));

            //Misc
            _config.AddSubMenu(new Menu("鏉傞」", "Misc"));
            _config.SubMenu("Misc").AddItem(new MenuItem("UseRM", "浣跨敤R鎶汉澶磡")).SetValue(true);
            _config.SubMenu("Misc").AddItem(new MenuItem("useigniteks", "浣跨敤鐐圭噧鎶汉澶磡")).SetValue(true);
            _config.SubMenu("Misc").AddItem(new MenuItem("usePackets", "浣跨敤灏佸寘")).SetValue(true);
            _config.SubMenu("Misc").AddItem(new MenuItem("Gap_E", "浣跨敤E|瀵圭獊鑴哥殑")).SetValue(true);

            //HitChance
            _config.AddSubMenu(new Menu("鍛戒腑鐜噟", "HitChance"));
            _config.SubMenu("HitChance").AddSubMenu(new Menu("楠氭壈", "Harass"));
            _config.SubMenu("HitChance").SubMenu("Harass").AddItem(new MenuItem("QchangeHar", "Q 鍛戒腑鐜噟").SetValue(
                new StringList(new[] {"浣巪", "涓瓅", "楂榺", "寰堥珮|"})));
            _config.SubMenu("HitChance").SubMenu("Harass").AddItem(new MenuItem("EchangeHar", "E 鍛戒腑鐜噟").SetValue(
                new StringList(new[] {"浣巪", "涓瓅", "楂榺", "寰堥珮|"})));
            _config.SubMenu("HitChance").SubMenu("Harass").AddItem(new MenuItem("RchangeHar", "R 鍛戒腑鐜噟").SetValue(
                new StringList(new[] {"浣巪", "涓瓅", "楂榺", "寰堥珮|"})));
            _config.SubMenu("HitChance").AddSubMenu(new Menu("杩炴嫑", "Combo"));
            _config.SubMenu("HitChance").SubMenu("Combo").AddItem(new MenuItem("Qchange", "Q 鍛戒腑鐜噟").SetValue(
                new StringList(new[] {"浣巪", "涓瓅", "楂榺", "寰堥珮|"})));
            _config.SubMenu("HitChance").SubMenu("Combo").AddItem(new MenuItem("Echange", "E 鍛戒腑鐜噟").SetValue(
                new StringList(new[] {"浣巪", "涓瓅", "楂榺", "寰堥珮|"})));
            _config.SubMenu("HitChance").SubMenu("Combo").AddItem(new MenuItem("Rchange", "R 鍛戒腑鐜噟").SetValue(
                new StringList(new[] {"浣巪", "涓瓅", "楂榺", "寰堥珮|"})));

            //Damage after combo:
            MenuItem dmgAfterComboItem = new MenuItem("DamageAfterCombo", "鏄剧ず缁勫悎杩炴嫑浼ゅ").SetValue(true);
            ...
            _config.SubMenu("Drawings").AddItem(new MenuItem("DrawQ", "鑼冨洿Q")).SetValue(true);
            ...
            Game.PrintChat("<font color='#881df2'>D-Kogmaw by Diabaths</font> 鍔犺浇鎴愬姛!姹夊寲by浜岀嫍!QQ缇361630847.");
