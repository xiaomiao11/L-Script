...
namespace KurisuBlitz
{
    ...
    internal class KurisuBlitz
    {
        ...
        {
            _menu = new Menu("Blitz浜岀嫍姹夊寲", "blitz", true);

            Menu blitzOrb = new Menu("Blitz: 璧扮爫", "orbwalker");
            ...
            Menu menuD = new Menu("鑼冨洿缁樺埗", "drawings");
            menuD.AddItem(new MenuItem("drawQ", "Q 鑼冨洿")).SetValue(new Circle(true, Color.FromArgb(150, Color.White)));
            menuD.AddItem(new MenuItem("drawR", "R 鑼冨洿")).SetValue(new Circle(true, Color.FromArgb(150, Color.White)));
            _menu.AddSubMenu(menuD);
            
            Menu menuG = new Menu("绁炰箣鎵嬶紙Q锛墊", "autograb");
            menuG.AddItem(new MenuItem("hitchance", "鍛戒腑鐜噟"))
                .SetValue(new StringList(new[] {"浣巪", "涓瓅", "楂榺"}, 2));
            menuG.AddItem(new MenuItem("dneeded", "鏈€杩戣窛绂籕")).SetValue(new Slider(255, 0, (int)Q.Range));
            menuG.AddItem(new MenuItem("dneeded2", "鏈€杩滆窛绂籕Q")).SetValue(new Slider((int)Q.Range, 0, (int)Q.Range));
            menuG.AddItem(new MenuItem("dashing", "鑷姩Q绉诲姩鐨勬晫浜簗")).SetValue(true);
            menuG.AddItem(new MenuItem("immobile", "鑷姩Q涓嶅姩鐨勬晫浜簗")).SetValue(true);
            menuG.AddItem(new MenuItem("hneeded", "涓嶇敓鍛藉€间綆浜巪%")).SetValue(new Slider(0));
                    ...
                    ObjectManager.Get<Obj_AI_Hero>()
                        .Where(
                            e =>
                                e.Team != _player.Team))
            {
                menuG.AddItem(new MenuItem("Q璁剧疆" + e.SkinName, e.SkinName))
                    .SetValue(new StringList(new[] {"涓峇 ", "姝ｅ父Q ", "鑷姩Q "}, 1));
            }
            _menu.AddSubMenu(menuG);
            Menu menuK = new Menu("鎶汉澶磡", "blitzks");
            menuK.AddItem(new MenuItem("killstealQ", "浣跨敤 Q")).SetValue(false);
            menuK.AddItem(new MenuItem("killstealE", "浣跨敤 E")).SetValue(false);
            menuK.AddItem(new MenuItem("killstealR", "浣跨敤 R")).SetValue(false);
            _menu.AddSubMenu(menuK);
            _menu.AddItem(new MenuItem("gapcloser", "鑷姩W璺熶笂鏁屼汉")).SetValue(true);
            _menu.AddItem(new MenuItem("interrupt", "涓柇娉曟湳")).SetValue(true);
            _menu.AddItem(new MenuItem("useE", "QQ涓嚜鍔‥")).SetValue(true);
            _menu.AddItem(new MenuItem("combokey", "杩炴嫑閿綅")).SetValue(new KeyBind(32, KeyBindType.Press));
            _menu.AddToMainMenu();
