...
namespace KurisuNidalee
{
    ...
    internal class KurisuNidalee
    {
        ...
        private void NidaMenu()
        {
            ...
            nidaKeys.AddItem(new MenuItem("useclear", "娓呭叺")).SetValue(new KeyBind(86, KeyBindType.Press));
            Config.AddSubMenu(nidaKeys);

            var nidaSpells = new Menu("娉曟湳", "spells");
            nidaSpells.AddItem(new MenuItem("hitchance", "鍛戒腑鐜噟")).SetValue(new StringList(new[] { "浣巪", "涓瓅", "楂榺" }, 2));
            nidaSpells.AddItem(new MenuItem("usehumanq", "浣跨敤鏍囨灙鎶曟幏")).SetValue(true);
            nidaSpells.AddItem(new MenuItem("usehumanw", "浣跨敤涓涙灄浼忓嚮")).SetValue(true);
            nidaSpells.AddItem(new MenuItem(" ", " "));
            nidaSpells.AddItem(new MenuItem("usecougarq", "浣跨敤鎺ㄥ埌")).SetValue(true);
            nidaSpells.AddItem(new MenuItem("usecougarw", "浣跨敤鐚涙墤")).SetValue(true);
            nidaSpells.AddItem(new MenuItem("pouncerange", "鏈€灏忕寷鎵戣窛绂粅")).SetValue(new Slider(125, 50, 300));
            nidaSpells.AddItem(new MenuItem("usecougare", "浣跨敤璞瑰舰鎬亅")).SetValue(true);
            nidaSpells.AddItem(new MenuItem("usecougarr", "鑷姩璞瑰舰鎬佽禋閽眧")).SetValue(true);
            Config.AddSubMenu(nidaSpells);

            var nidaHeals = new Menu("娌绘剤寮曟搸", "hengine");
            nidaHeals.AddItem(new MenuItem("usedemheals", "鍚敤")).SetValue(true);
            ...
            nidaHeals.AddItem(new MenuItem("healmanapct", "绂佺敤娌绘剤锛堥瓟娉曞€硷級%")).SetValue(new Slider(40));
            Config.AddSubMenu(nidaHeals);


            var nidaHarass = new Menu("楠氭壈", "harass");
            nidaHarass.AddItem(new MenuItem("usehumanq2", "浣跨敤鏍囨灙鎶曟幏")).SetValue(true);
            nidaHarass.AddItem(new MenuItem("humanqpct", "绂佺敤Q锛堟硶鍔涘€硷級%")).SetValue(new Slider(70));
            Config.AddSubMenu(nidaHarass);

            var nidaClear = new Menu("娓呯嚎", "laneclear");
            nidaClear.AddItem(new MenuItem("clearhumanq", "浣跨敤鏍囨灙鎶曟幏")).SetValue(false);
            nidaClear.AddItem(new MenuItem(" ", " "));
            nidaClear.AddItem(new MenuItem("clearcougarq", "浣跨敤鎺ㄥ埌")).SetValue(true);
            nidaClear.AddItem(new MenuItem("clearcougarw", "浣跨敤鐚涙墤")).SetValue(true);
            nidaClear.AddItem(new MenuItem("clearcougare", "浣跨敤璞瑰舰鎬亅")).SetValue(true);
            nidaClear.AddItem(new MenuItem("clearcougarr", "鑷姩璞瑰舰鎬佹竻绾縷")).SetValue(false);
            nidaClear.AddItem(new MenuItem("clearpct", "绂佺敤娓呯嚎锛堟硶鍔涘€硷級%")).SetValue(new Slider(55));
            Config.AddSubMenu(nidaClear);

            var nidaJungle = new Menu("娓呴噹", "jungleclear");
            nidaJungle.AddItem(new MenuItem("jghumanq", "浣跨敤鏍囨灙鎶曟幏")).SetValue(true);
            nidaJungle.AddItem(new MenuItem("jghumanw", "浣跨敤涓涙灄浼忓嚮")).SetValue(true);
            nidaJungle.AddItem(new MenuItem(" ", " "));
            nidaJungle.AddItem(new MenuItem("jgcougarq", "浣跨敤鎺ㄥ埌")).SetValue(true);
            nidaJungle.AddItem(new MenuItem("jgcougarw", "浣跨敤鐚涙墤")).SetValue(true);
            nidaJungle.AddItem(new MenuItem("jgcougare", "浣跨敤璞瑰舰鎬亅")).SetValue(true);
            nidaJungle.AddItem(new MenuItem("jgcougarr", "鑷姩璞瑰舰鎬佹竻閲巪")).SetValue(true);
            nidaJungle.AddItem(new MenuItem("jgrpct", "绂佺敤娓呴噹锛堟硶鍔涘€硷級%")).SetValue(new Slider(55, 0, 100));
            Config.AddSubMenu(nidaJungle);

            var nidaMisc = new Menu("鏉傞」", "nidamisc");
            nidaMisc.AddItem(new MenuItem("usedfg", "浣跨敤 閬撳叿")).SetValue(true);
            nidaMisc.AddItem(new MenuItem("usebork", "浣跨敤 鐮磋触")).SetValue(true);
            nidaMisc.AddItem(new MenuItem("usebw", "浣跨敤灏忕牬璐")).SetValue(true);
            nidaMisc.AddItem(new MenuItem("useclaim", "浣跨敤鍐伴湝濂崇殗鐨勬棬浠")).SetValue(true);
            nidaMisc.AddItem(new MenuItem("useks", "鎶汉澶磡")).SetValue(true);
            nidaMisc.AddItem(new MenuItem("swfks", "鎶汉澶磡鍒囪惫褰㈡€亅")).SetValue(false);
            Config.AddSubMenu(nidaMisc);

            var nidaD = new Menu("鑼冨洿", "drawings");
            nidaD.AddItem(new MenuItem("drawQ", "鑼冨洿 Q")).SetValue(new Circle(true, Color.FromArgb(150, Color.White)));
            nidaD.AddItem(new MenuItem("drawW", "鑼冨洿 W")).SetValue(new Circle(true, Color.FromArgb(150, Color.White)));
            nidaD.AddItem(new MenuItem("drawE", "鑼冨洿 E")).SetValue(new Circle(true, Color.FromArgb(150, Color.White)));
            nidaD.AddItem(new MenuItem("drawcds", "鏄剧ず鎶€鑳藉喎鍗磡")).SetValue(true);
            ...
            Game.PrintChat("<font color=\"#FFAF4D\">[</font><font color=\"#FFA333\">Nidalee</font><font color=\"#FFAF4D\">]</font><font color=\"#FF8C00\"> - <u>the Bestial Huntress Rev106 鍔犺浇鎴愬姛!姹夊寲by浜岀嫍!QQ缇361630847 </u>  </font>- Kurisu");
