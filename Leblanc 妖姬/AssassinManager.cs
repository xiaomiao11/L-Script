...
namespace Leblanc
{
    internal class AssassinManager
    {
        ...
        {
            Program.MenuPlayOptions.AddSubMenu(new Menu("鍒哄缁忕悊", "AssassinManager"));
            Program.MenuPlayOptions.SubMenu("AssassinManager")
                .AddItem(new MenuItem("AssassinActive", "鏈夋晥鏉€鍒簗").SetValue(true));
            Program.MenuPlayOptions.SubMenu("AssassinManager")
                .AddItem(new MenuItem("AssassinSetClick", "鐐瑰嚮娣诲姞|鍒犻櫎").SetValue(true));
            Program.MenuPlayOptions.SubMenu("AssassinManager")
                .AddItem(
                    new MenuItem("AssassinRangeColor", "鏈夋晥棰滆壊鑼冨洿").SetValue(new Circle(true,
                        Color.GreenYellow)));
            Program.MenuPlayOptions.SubMenu("AssassinManager")
                .AddItem(
                    new MenuItem("AssassinInRangeColor", "鏁屼汉棰滆壊鑼冨洿").SetValue(new Circle(true,
                        Color.GreenYellow)));
            Program.MenuPlayOptions.SubMenu("AssassinManager")
                .AddItem(
                    new MenuItem("AssassinInCloseColor", "鏈€杩戞晫浜洪鑹瞸").SetValue(new Circle(true,
                        Color.DarkSeaGreen)));
            Program.MenuPlayOptions.SubMenu("AssassinManager")
                .AddItem(
                    new MenuItem("AssassinReset", "閲嶇疆鍒烘潃鍒楄〃").SetValue(new KeyBind("J".ToCharArray()[0],
                        KeyBindType.Press)));
            ...
            Program.MenuPlayOptions.SubMenu("AssassinManager")
                .AddItem(new MenuItem("AssassinRange", "鏈夋晥鑼冨洿")).SetValue(new Slider(1000, 2000));
