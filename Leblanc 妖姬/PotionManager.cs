...
namespace Leblanc
{
    internal class PotionManager
    {
        ...
        private void Load()
        {
            ...
            Program.MenuExtras.AddSubMenu(new Menu("鑽按缁忕悊", "PotionManager"));

            Program.MenuExtras.SubMenu("PotionManager").AddSubMenu(new Menu("鐢熷懡鍊紎", "Health"));
            Program.MenuExtras.SubMenu("PotionManager").SubMenu("Health").AddItem(new MenuItem("HealthPotion", "浣跨敤鐢熷懡鑽按").SetValue(true));
            Program.MenuExtras.SubMenu("PotionManager").SubMenu("Health").AddItem(new MenuItem("HealthPercent", "HP瑙﹀彂鐧惧垎姣攟").SetValue(new Slider(30)));

            Program.MenuExtras.SubMenu("PotionManager").AddSubMenu(new Menu("榄旀硶鍊紎", "Mana"));
            Program.MenuExtras.SubMenu("PotionManager").SubMenu("Mana").AddItem(new MenuItem("ManaPotion", "浣跨敤鍥炶摑鑽按").SetValue(true));
            Program.MenuExtras.SubMenu("PotionManager").SubMenu("Mana").AddItem(new MenuItem("ManaPercent", "MP瑙﹀彂鐧惧垎姣攟").SetValue(new Slider(30)));
