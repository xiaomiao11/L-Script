...
namespace Marksman
{
    internal class Tristana : Champion
    {
        ...
        public Tristana()
        {
            Utils.PrintMessage("Tristana loaded 姹夊寲by浜岀嫍!QQ缇361630847.");
        ...
        public override bool HarassMenu(Menu config)
        {
            ...
            config.AddItem(
                new MenuItem("UseETH" + Id, "浣跨敤 E (鎸佺画)").SetValue(new KeyBind("H".ToCharArray()[0],
                    KeyBindType.Toggle)));
            return true;
        }
        ...
        {
            config.AddItem(
                new MenuItem("DrawE" + Id, "E 鑼冨洿").SetValue(new Circle(true, Color.CornflowerBlue)));
            return true;
        }

        public override bool MiscMenu(Menu config)
        {
            config.AddItem(new MenuItem("UseRM" + Id, "浣跨敤R鎶汉澶磡").SetValue(true));
            config.AddItem(new MenuItem("UseRMG" + Id, "浣跨敤R闃茬獊杩泑").SetValue(true));
            config.AddItem(new MenuItem("UseRMI" + Id, "浣跨敤R涓柇娉曟湳").SetValue(true));
            return true;
        }
