...
namespace MasterActivator
{
    internal class MActivator
    {
        ...
        {
            // Boas vindas
            Game.PrintChat("<font color='#3BB9FF'>MasterActivator - by Crisdmc 鍔犺浇鎴愬姛!姹夊寲by浜岀嫍!QQ缇36163084 - </font>Loaded");
        ...
        {
            Config = new Menu("M娲诲寲鍓倈 浜岀嫍姹夊寲", "masterActivator", true);

            Config.AddSubMenu(new Menu("鍑€鍖栧櫒锛堟按閾剁瓑锛墊", "purifiers"));
            ...
            Config.SubMenu("purifiers").AddItem(new MenuItem("defJustOnCombo", "鍦ㄨ繛鎷涗腑浣跨敤")).SetValue(true);

            Config.AddSubMenu(new Menu("鍑€鍖栧紓甯哥姸鎬亅", "purify"));
            ...
            Config.SubMenu("purify").AddItem(new MenuItem("flee", "閫冭窇")).SetValue(true);
            ...
            Config.AddSubMenu(new Menu("鎯╂垝瀵硅薄", "smiteCfg"));
            ...
            else
            {
                ...
                Config.SubMenu("smiteCfg").AddItem(new MenuItem("LizardElder", "绾uff")).SetValue(true);
                ...
                Config.SubMenu("smiteCfg").AddItem(new MenuItem("GreatWraith", "澶у菇鐏祙")).SetValue(false);
                Config.SubMenu("smiteCfg").AddItem(new MenuItem("Wraith", "灏忓菇鐏祙")).SetValue(false);
                Config.SubMenu("smiteCfg").AddItem(new MenuItem("Golem", "鐭冲ご浜簗")).SetValue(false);
                Config.SubMenu("smiteCfg").AddItem(new MenuItem("GiantWolf", "澶х嫾")).SetValue(false);

            }
            Config.SubMenu("smiteCfg").AddItem(new MenuItem("dSmite", "鎯╂垝鑼冨洿")).SetValue(true);
            Config.SubMenu("smiteCfg").AddItem(new MenuItem("justAS", "濮嬬粓寮€鍚瘄")).SetValue(false);

            Config.AddSubMenu(new Menu("鏀诲嚮閬撳叿锛堝菇姊︾瓑锛墊", "offensive"));
            createMenuItem(ignite, 100, "offensive", false, false);
            Config.SubMenu("offensive").AddItem(new MenuItem("overIgnite", "鍦ㄧ偣鐕冨悗浣跨敤")).SetValue(false);
            ...
            Config.AddSubMenu(new Menu("鍔犺|鍔犺摑", "regenerators"));
            ...
            Config.SubMenu("combo").AddItem(new MenuItem("comboModeActive", "娲诲寲")).SetValue(new KeyBind(32, KeyBindType.Press, true));

            // Target selector
            Config.AddSubMenu(new Menu("鐩爣閫夋嫨", "targetSelector"));
            Config.SubMenu("targetSelector").AddItem(new MenuItem("targetMode", "")).SetValue(new StringList(new[] { "琛€閲忔渶灏戠殑", "鏈€楂楢D杈撳嚭鐨剕", "鏈€楂楢P杈撳嚭鐨剕", "鏈€杩戠殑闃熷弸", "鎺ヨ繎榧犳爣鐨剕", "鑷姩鍑€鍖東", "鏀诲嚮鍔涙渶灏戠殑", "杈呭姪" }, 0));

            Config.AddItem(new MenuItem("predict", "棰勬祴 DMG")).SetValue(true);
            Config.AddItem(new MenuItem("enabled", "鍔熻兘寮€鍚痮r鍏抽棴")).SetValue(true);
