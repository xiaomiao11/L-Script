    ...
    internal class Humanizer : Base
    {
        ...
        private void OnGameLoad(EventArgs args)
        {
            try
            {
                ...
                delayMenu.AddItem(new MenuItem(Name + "DelaySpells", "娉曟湳(ms)").SetValue(new Slider(50, 0, 250)));
                delayMenu.AddItem(new MenuItem(Name + "DelayMovement", "绉诲姩(ms)").SetValue(new Slider(50, 0, 250)));

                Menu.AddSubMenu(delayMenu);

                var eMenuItem = new MenuItem(Name + "Enabled", "鎵ц閿綅").SetValue(true);
