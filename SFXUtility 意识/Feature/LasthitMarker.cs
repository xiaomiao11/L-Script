    ...
    internal class LasthitMarker : Base
    {
        ...
        private void OnGameLoad(EventArgs args)
        {
            try
            {
                ...
                var drawingHpBarMenu = new Menu("琛€閲忓垎鏍紎", Name + "HPBar");
                drawingHpBarMenu.AddItem(
                    new MenuItem(Name + "DrawingHpBarKillableColor", "鍑绘潃棰滆壊").SetValue(Color.Green));
                drawingHpBarMenu.AddItem(
                    new MenuItem(Name + "DrawingHpBarUnkillableColor", "涓嶈兘鍑绘潃棰滆壊").SetValue(Color.White));
                drawingHpBarMenu.AddItem(
                    new MenuItem(Name + "DrawingHpBarLinesThickness", "绾垮帤搴").SetValue(new Slider(1, 1, 10)));
                drawingHpBarMenu.AddItem(new MenuItem(Name + "DrawingHpBarEnabled", "鎵ц閿綅").SetValue(true));

                var drawingCirclesMenu = new Menu("鍦坾", Name + "Circle");
                drawingCirclesMenu.AddItem(
                    new MenuItem(Name + "DrawingCircleColor", "鍦坾棰滆壊").SetValue(Color.Fuchsia));
                drawingCirclesMenu.AddItem(
                    new MenuItem(Name + "DrawingCircleRadius", "鍦坾鑼冨洿锛堝崐寰勶級").SetValue(new Slider(30)));
