        ...
        private void TimersLoaded(object o)
        {
            try
            {
                if (o is Timers && (o as Timers).Menu != null)
                {
                    ...
                    Menu = new Menu("鍏嶄簬椤箌", "Immune");

                    var drawingMenu = new Menu("缁樺埗", Name + "Drawing");
                    drawingMenu.AddItem(new MenuItem(Name + "DrawingAllyColor", "鐩熷弸棰滆壊").SetValue(Color.DarkRed));
                    drawingMenu.AddItem(new MenuItem(Name + "DrawingEnemyColor", "鏁屾柟棰滆壊").SetValue(Color.DarkRed));

                    Menu.AddSubMenu(drawingMenu);

                    Menu.AddItem(new MenuItem(Name + "ShowAlly", "鏄剧ず鐩熷弸").SetValue(true));
                    Menu.AddItem(new MenuItem(Name + "ShowEnemy", "鏄剧ず鏁屾柟").SetValue(true));
