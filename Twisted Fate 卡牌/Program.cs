...
namespace TwistedFate
{
    internal class Program
    {
        ...
        private static void Game_OnGameLoad(EventArgs args)
        {
            ...
            //Make the menu
            Config = new Menu("TwistedFate浜岀嫍姹夊寲", "TwistedFate", true);
            ...
            /* Q */
            var q = new Menu("Q-涓囪兘鐗寍", "Q");
            q.AddItem(new MenuItem("AutoQI", "鑷姩Q(瀹氫綇鐨勶級").SetValue(true));
            q.AddItem(new MenuItem("AutoQD", "鑷姩Q(绉诲姩鐨勶級").SetValue(true));
            q.AddItem(new MenuItem("CastQ", "鎵嬪姩Q锛堣嚜鍔ㄧ瀯鍑嗭級").SetValue(new KeyBind("U".ToCharArray()[0], KeyBindType.Press)));
            Config.AddSubMenu(q);

            /* W */
            var w = new Menu("W-閫夌墝", "W");
            w.AddItem(
                new MenuItem("SelectYellow", "鍒囬粍鐗寍").SetValue(new KeyBind("W".ToCharArray()[0],
                    KeyBindType.Press)));
            w.AddItem(
                new MenuItem("SelectBlue", "鍒囪摑鐗寍").SetValue(new KeyBind("E".ToCharArray()[0], KeyBindType.Press)));
            w.AddItem(
                new MenuItem("SelectRed", "鍒囩孩鐗寍").SetValue(new KeyBind("T".ToCharArray()[0], KeyBindType.Press)));
            Config.AddSubMenu(w);

            var r = new Menu("R-鍛借繍", "R");
            r.AddItem(new MenuItem("AutoY", "澶ф嫑鍚庤嚜鍔ㄥ垏榛勭墝").SetValue(true));
            Config.AddSubMenu(r);

            var misc = new Menu("鏉傞」", "Misc");
            misc.AddItem(new MenuItem("PingLH", "鑷姩R锛堝彲鍑绘潃鐨勬晫鏂癸級").SetValue(true));
            ...
            Drawings.AddItem(new MenuItem("Rcircle2", "R 鑼冨洿 (灏忓湴鍥緗)").SetValue(new Circle(true, Color.FromArgb(255, 255, 255, 255))));
            Drawings.AddItem(dmgAfterComboItem);
            Config.AddSubMenu(Drawings);

            Config.AddItem(new MenuItem("Combo", "鎵ц杩炴嫑锛侊紒").SetValue(new KeyBind(32, KeyBindType.Press)));
